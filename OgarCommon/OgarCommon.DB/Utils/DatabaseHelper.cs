...
using System.Data;
using System.Text;
using OgarCommon.DB.Core;

namespace OgarCommon.DB.Utils
{
    public static class DatabaseHelper
    {
        ...
        public static string CreateConnectionString(DatabaseType dbType, string dbHost, string dbName, string dbUser, string dbPwd)
        {
            switch (dbType)
            {
                ...
                case DatabaseType.Ole:
                    return CreateOleConnectionString(dbHost,dbName,dbUser,dbPwd);
                case DatabaseType.SQLite:
                    return CreateSQLiteConnectionString(dbHost, dbName, dbUser, dbPwd);
                default:
        ...
        public static string CreateSQLiteConnectionString(string dbHost,string dbName,string dbUser,string dbPwd)
        {
            return string.Format("Data Source={0};Version=3;", dbName);
        }

        public static string CreateOleConnectionString(string dbHost,string dbName,string dbUser,string dbPwd)
        {
            var sb = new StringBuilder();
            sb.AppendFormat("password={0};User ID={1};Data Source=(DESCRIPTION=(ADDRESS_LIST=(ADDRESS=(PROTOCOL=TCP)(HOST={2})(PORT=1521)))(CONNECT_DATA=(SERVER=DEDICATED)(SERVICE_NAME={3})))",
                dbPwd, dbUser, dbHost, dbName);
